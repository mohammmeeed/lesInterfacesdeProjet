...
import com.example.lesinterfacesdeprojetl3.databinding.ActivityMap2Binding;
...
public class map2 extends FragmentActivity implements OnMapReadyCallback {
    ...
    public void onMapReady(GoogleMap googleMap) {
        mMap = googleMap;
        Intent lc = getIntent();
        double lat = lc.getDoubleExtra("lat",0.0);
        double lng = lc.getDoubleExtra("lng",0.0);

        // Add a marker in Sydney and move the camera
        LatLng park = new LatLng(lat, lng);
        mMap.addMarker(new MarkerOptions().position(park));
